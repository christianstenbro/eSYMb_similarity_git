...
```{r}
# reading data from session 1
data <- read_csv(here("data", "study1", "raw_data", "all_apps_wide-2025-06-30 (1).csv"))

# loading otree page time data
pagetimes <- read_csv(here("data", "study1", "raw_data", "PageTimes-2025-06-30 (1).csv"))
```
...
data <- data %>% mutate(session_date = "2025-06-30",
                   session_code = "study_1_session_1")
```
...
df <- df %>% arrange(sequential_ID, image_indices)
```
...
```{r}
file_name = "processed_data_stim_study_1_28_participants"
extension = ".csv"
path = here("data", "study1", "processed_data", paste0(file_name, extension))
