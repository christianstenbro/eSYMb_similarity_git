		...
		function finalizeline(e) {
			...
			if (currentAction == "newline") {
				...
				if (length  < 25) {
					...
					line.style.width = lineLength + 'px'; // always same size
					lineCount++;
				}
...
function validate_drawing(sendfun) {
	...
	valid_linecount = (lineCount == 6);
	// valid_pattern()

	if (valid_linecount && valid_pattern()) {  // data looks valid, lets submit
...
function liveRecv(data) {
    ...
    switch(data.event) {
        case 'drawing_accepted':
		    submit_send();
            break;
        case 'drawing_rejected':
		    deleteDrawing();
            error_message("Your drawing did not follow the instructions. Please try again.", reset = true);
            break;
        case 'pong':
            schedule_ping();
            break;
        case 'init_con0':
            if ("drawing" in data) {
                // restore data for saving
				document.querySelector("#draw #surface #canvas").style.backgroundImage="url("+data['drawing']+")";
			    document.querySelector("#linecount").value = 6;
                lineCount = 6;
                start_waiting_for_moderation();
            }
            break;
        case 'init_con1':
            if ("drawing" in data & "pattern" in data) {
                // restore data for saving
				document.querySelector("#draw #surface #canvas").style.backgroundImage="url("+data['drawing']+")";
			    document.querySelector("#linecount").value = 6;
                lineCount = 6;
                document.querySelector("#pattern input[value='"+data['pattern']+"']").checked = true
                grayPatterns();
                start_waiting_for_moderation();
...
function start_waiting_for_moderation() {
	document.querySelector("button.otree-btn-next").setAttribute("disabled", "disabled");
    nextbuttonText = document.querySelector(".otree-btn-next").innerHTML;
    document.querySelector(".otree-btn-next").innerHTML='<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>';
	schedule_ping();
}
...
function submit_send() {
    document.querySelector("input#linecount").value = 6;
    // lineCount = 6;
