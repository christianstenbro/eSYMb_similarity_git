...
```{r}
# reading pilot data
data <- read_csv("/Users/christianstenbro/AU/eSYMb/Similarity Rating Exp/Pilot/otree_data_new_structure/SimilarityRatingExperiment_2025-06-26.csv")
```
```{r}
# loading otree page time data
pagetimes <- read_csv("/Users/christianstenbro/AU/eSYMb/Similarity Rating Exp/Pilot/otree_data_new_structure/PageTimes-2025-06-26.csv")
```
...
```{r}
data$subsession.round_number

round_vector <- rep(i, length(study_number))
```

```{r}
round_number <- list()

round_number <- append(round_number, rep(1, 20))

unlist(round_number)

length(df$sequential_ID)

image_indices <- list()

image_indices <- append(image_indices, list(fromJSON(data$player.imageIndices[i])))

length(image_indices[[1]])



round_number <- append(round_number, rep(1, length(image_indices[[1]])))

length(unlist(round_number))
length(unlist(image_indices))


length(fromJSON(data$player.imageIndices[1]))
```
...
```{r}
subset_with_attention_check_status(1)
...
plot(att_check_subset$passed_checks,
     ...
     ylim = c(0,6),
     main = "Passed attention checks x participant IDs")
```
